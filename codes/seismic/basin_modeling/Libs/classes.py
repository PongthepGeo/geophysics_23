...
class SeismicWavefield:
	...
	def wavefield(self, nt):
		source_amplitudes = deepwave.wavelets.ricker(self.freq, nt, self.dt, self.peak_time).reshape(1, 1, -1).to(dtype=torch.float32, device=self.device)
...
class Migration:
	...
	def run_inversion(self, n_epochs, shot_interval, n_shots):
		observed_scatter_files = [os.path.join(self.npy_folder, f'shot_pixel_{i * shot_interval:04d}.npy') for i in range(n_shots)]
		observed_scatter_masked = [torch.tensor(np.load(f), device=self.device) for f in observed_scatter_files]

		for epoch in tqdm(range(n_epochs), desc="Epochs"):
			epoch_loss = 0
			for shot_index in tqdm(range(n_shots), desc="Shots"):
				self.optimizer.zero_grad()
				current_source_position = shot_index * shot_interval
				source_locations = torch.zeros(1, 1, 2, dtype=torch.float32, device=self.device)
				...
				loss.backward()
				self.optimizer.step()
				print(f'Shot {shot_index}, Iteration Loss: {loss.item()}')
	...
	def plot_migration(self, output_folder, output_migration_name, clip_percent=99, save=False):
		...
		# migration_data = np.load('image_out/migration_sm70.npy')
		migration_data -= np.mean(migration_data)
		migration_data /= np.max(np.abs(migration_data))
		migration_data_clipped = migration_data[:, 350:]  # Clip top image
		# migration_data_clipped = migration_data[:, :]  # Clip top image
		migration_data_laplace = ndimage.laplace(migration_data_clipped)
		vmax_clip, vmin_clip = U.clip(migration_data_laplace, clip_percent)

		plt.figure()
		plt.imshow(migration_data_laplace.T, aspect='auto', cmap='gray', vmin=vmin_clip, vmax=vmax_clip)
		plt.xlabel('Distance (pixel)')
		plt.ylabel('Depth (m)')
		plt.title('Migration with Laplace Filter')
		if save:
			save_name = output_folder + "/" + output_migration_name
			plt.savefig(save_name, format='svg', bbox_inches='tight', pad_inches=0, transparent=True)
